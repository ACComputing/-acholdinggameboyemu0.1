...
class DeepSeekGB:
    ...
    def _dispatch(self, op):
        ...
        # ----- 16-bit INC/DEC -----
            ...
            self.b = (bc>>8)&0xFF; self.c = bc&0xFF; self.cycles += 8
        elif op == 0x13:
            de = ((self.d<<8)|self.e) + 1
            self.d = (de>>8)&0xFF; self.e = de&0xFF; self.cycles += 8
        elif op == 0x23:
            hl = ((self.h<<8)|self.l) + 1
            self.h = (hl>>8)&0xFF; self.l = hl&0xFF; self.cycles += 8
        elif op == 0x33:
        ...
        # ----- Jumps / Calls / Returns (FIXED SYNTAX) -----
        elif op == 0x18:
            ...
            self.pc = self._read_byte(self.pc) | (self._read_byte(self.pc+1) << 8)
            self.cycles += 16
        elif op == 0xE9:
        ...
        # ----- Stack -----
            ...
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.c)
            self.cycles += 16
        elif op == 0xD5:
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.d)
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.e)
            self.cycles += 16
        elif op == 0xE5:
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.h)
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.l)
            self.cycles += 16
        elif op == 0xF5:
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.a)
            self.sp = (self.sp-1)&0xFFFF; self._write_byte(self.sp, self.f)
            self.cycles += 16
        elif op == 0xC1:
            self.c = self._read_byte(self.sp); self.sp += 1
            self.b = self._read_byte(self.sp); self.sp += 1
            self.cycles += 12
        elif op == 0xD1:
            self.e = self._read_byte(self.sp); self.sp += 1
            self.d = self._read_byte(self.sp); self.sp += 1
            self.cycles += 12
        elif op == 0xE1:
            self.l = self._read_byte(self.sp); self.sp += 1
            self.h = self._read_byte(self.sp); self.sp += 1
            self.cycles += 12
        elif op == 0xF1:
            self.f = self._read_byte(self.sp) & 0xF0; self.sp += 1
            self.a = self._read_byte(self.sp); self.sp += 1
            self.cycles += 12

        # ----- Misc -----
        elif op == 0x76:
            self.halted = True; self.cycles += 4
        elif op == 0xF3:
            ...
            val = self._read_byte(self.pc); self.pc+=1; self._add_a(val); self.cycles+=4
        elif op == 0xCE:
            val = self._read_byte(self.pc); self.pc+=1; self._adc_a(val); self.cycles+=4
        elif op == 0xD6:
            val = self._read_byte(self.pc); self.pc+=1; self._sub(val); self.cycles+=4
        elif op == 0xDE:
            val = self._read_byte(self.pc); self.pc+=1; self._sbc(val); self.cycles+=4
        elif op == 0xE6:
            val = self._read_byte(self.pc); self.pc+=1; self._and(val); self.cycles+=4
        elif op == 0xEE:
            val = self._read_byte(self.pc); self.pc+=1; self._xor(val); self.cycles+=4
        elif op == 0xF6:
            val = self._read_byte(self.pc); self.pc+=1; self._or(val); self.cycles+=4
        elif op == 0xFE:
            val = self._read_byte(self.pc); self.pc+=1; self._cp(val); self.cycles+=4
            ...
            self.a ^= 0xFF; self._set_flag(6,1); self._set_flag(5,1); self.cycles+=4
        elif op == 0x3F:
            self._set_flag(6,0); self._set_flag(5,0); self._set_flag(4, not self._get_flag(4))
            self.cycles+=4
        elif op == 0x37:
            self._set_flag(6,0); self._set_flag(5,0); self._set_flag(4,1); self.cycles+=4
        elif op == 0x08:
            addr = self._read_byte(self.pc) | (self._read_byte(self.pc+1)<<8); self.pc+=2
            self._write_byte(addr, self.sp & 0xFF); self._write_byte(addr+1, self.sp>>8)
            self.cycles+=20
        elif op == 0xF9:
    ...
    def load_rom(self, data: bytes):
        ...
        self.rom_banks = 2 << rom_size if rom_size <= 8 else 2
        if data[0x0147] in (1,2,3):
            self.mbc1_rom_bank = 1
...
class DeepSeekEmu:
    ...
    def load(self):
        ...
        if p:
            ...
            self.status.config(text=f"ROM loaded: {p.split('/')[-1]}")
    def run(self): self.running = True; self.status.config(text="Running — DeepSeek mode active")
    def pause(self): self.running = False; self.status.config(text="Paused — DeepSeek idle")
    def reset(self): self.core.reset(); self.status.config(text="Reset — DeepSeek rebooting")
    def draw(self, fb):
